...
logger.info(f"kb_inclusion_tag_key: {kb_inclusion_tag_key}")
...
# @mcp.tool(name='GetKnowledgeBases')
# async def get_knowledge_bases_tool() -> str:
#     """Get all available Amazon Bedrock Knowledge Bases and their data sources.
#     """
#     kb_list = json.dumps(await discover_knowledge_bases(kb_agent_mgmt_client, kb_inclusion_tag_key))
#     logger.info(f"kb_list: {kb_list}")
#     return kb_list
...
async def query_knowledge_bases_tool(
    ...
    logger.info(f"number_of_results: {number_of_results}")
    logger.info(f"reranking: {reranking}")
    logger.info(f"reranking_model_name: {reranking_model_name}")
    logger.info(f"data_source_ids: {data_source_ids}")
